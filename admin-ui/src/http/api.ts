...
export const createProduct = (product: FormData) => api.post(`${CATALOG_SERVICE}/products`, Object.fromEntries(product.entries()), { headers: { 'Content-Type': 'application/json' } });
export const getCategory = (id: string) => api.get(`${CATALOG_SERVICE}/categories/${id}`);
export const updateProduct = (product: FormData, id: string) => api.put(`${CATALOG_SERVICE}/products/${id}`, Object.fromEntries(product.entries()));
